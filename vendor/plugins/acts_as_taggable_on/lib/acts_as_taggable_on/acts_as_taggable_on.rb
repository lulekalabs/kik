    ...
    module TaggableOn
      ...
      module SingletonMethods
        ...
        def find_options_for_tag_counts(options = {})
          options.assert_valid_keys :start_at, :end_at, :conditions, :at_least, :at_most, :order, :limit, :on, :id, :tags
          
          tags = options[:tags].is_a?(Array) ? TagList.new(options[:tags].map(&:to_s)) : TagList.from(options[:tags])
          ...
          unless tags.empty?
            # alias
            taggings_alias, tags_alias = "#{Tagging.table_name}", "#{Tag.table_name}_tags"

            conditions << tags.map { |t| sanitize_sql(["#{tags_alias}.name LIKE ?", t]) }.join(" OR ")
            
            joins << "LEFT OUTER JOIN #{Tag.table_name} #{tags_alias} ON #{tags_alias}.id = #{taggings_alias}.tag_id"
          end
